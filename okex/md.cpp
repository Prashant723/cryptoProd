...
int session::getSubscriptionStrings() {
    std::ifstream symbolFile("okexSymbols.txt");
    std::string symbol;
    int symbolsNo=0;
    int limit = 100;
    

  //  std::string currentSubscription = '{"op":"subscribe","args": [';
  std::string currentSubscription; 

    if (symbolFile.is_open()) {
        currentSubscription = "{\"op\":\"subscribe\",\"args\":[";
        while (std::getline(symbolFile, symbol)) {
                // Symbol matches the requested type
                currentSubscription += "{\"channel\":\"books5\",\"instId\":";
                currentSubscription += '"' + std::string(symbol) + '"' + "},";
    ...
    return subscriptionStrings.size();
}
...
void connection_metadata::on_open(int id, client * c ,websocketpp::connection_hdl hdl) {
    m_status = "Open";
    std::string subscribe = session_->subscriptionStrings[id-1];
    // std::cout<<subscribe<<std::endl;
    client::connection_ptr con = c->get_con_from_hdl(hdl);
    con->send(subscribe, websocketpp::frame::opcode::text);
...
void connection_metadata::on_message(int id, client::message_ptr msg) {
    ...
    std::string symbol = document["arg"]["instId"].GetString(); 
...
int websocket_endpoint::connect() {
    ...
    int n = session_->getSubscriptionStrings();
